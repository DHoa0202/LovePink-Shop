...
public class RestProduct {
	...
	public ResponseEntity<Map<String, List>> getMap(Model model) {
		...
		map.put("categories", categoryDAO.findAll());
		map.put("products", productDAO.findAll());
	...
	public ResponseEntity<Product> update(@RequestBody Product product) {
		Optional<Product> optional = productDAO.findById(product.getId());
		try {
			if (!optional.isEmpty()) return ResponseEntity.ok(productDAO.save(product));
		} catch (Exception e) {
